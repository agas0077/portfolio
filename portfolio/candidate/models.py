...
class Candidate(models.Model):
    ...
    image = models.ImageField(
        "Фотография", upload_to="user_image/", blank=True, null=True
    )
    ...
    about_skills = models.TextField("О навыках", blank=True, null=True)
...
class Language(models.Model):
    ...
    user = models.ForeignKey(
        Candidate, on_delete=models.CASCADE, related_name="languages"
    )
    title = models.CharField("Название", max_length=200)
...
class Recomendation(models.Model):
    user = models.ForeignKey(
        Candidate, on_delete=models.CASCADE, related_name="recomendations"
    )
    ...
    author_job_title = models.CharField(
        "Должность автора рекомендации", max_length=200
    )
...
class OtherProject(models.Model):
    user = models.ForeignKey(
        Candidate, on_delete=models.CASCADE, related_name="other_projects"
    )
    title = models.CharField("Название", max_length=200)
    description = models.TextField("Описание")
...
class Project(models.Model):
    user = models.ForeignKey(
        Candidate, on_delete=models.CASCADE, related_name="projects"
    )
